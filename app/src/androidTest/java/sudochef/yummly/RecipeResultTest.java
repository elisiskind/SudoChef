package sudochef.yummly;

import junit.framework.TestCase;

public class RecipeResultTest extends TestCase {

    public void testGetCourses() throws Exception {

    }

    public void testSetCourses() throws Exception {

    }

    public void testGetCuisines() throws Exception {

    }

    public void testSetCuisines() throws Exception {

    }

    public void testGetHolidays() throws Exception {

    }

    public void testSetHolidays() throws Exception {

    }

    public void testGetRating() throws Exception {

    }

    public void testSetRating() throws Exception {

    }

    public void testGetId() throws Exception {

    }

    public void testSetId() throws Exception {

    }

    public void testGetImageURL() throws Exception {

    }

    public void testSetThumbnail() throws Exception {

    }

    public void testGetSource() throws Exception {

    }

    public void testSetSource() throws Exception {

    }

    public void testGetTime() throws Exception {

    }

    public void testSetTime() throws Exception {

    }

    public void testGetIngredients() throws Exception {

    }

    public void testSetIngredients() throws Exception {

    }

    public void testGetName() throws Exception {

    }

    public void testSetName() throws Exception {

    }

    public void testPrint() throws Exception {

    }

    public void testDescriptionString() throws Exception {

    }

    public void testLoadThumb() throws Exception {

    }

    public void testGetThumb() throws Exception {

    }
}